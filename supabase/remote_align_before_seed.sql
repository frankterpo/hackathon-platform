-- Prefer the CLI instead: `npm run supabase:apply` (migrations + seed on linked project).
-- Use this file in the SQL Editor only if you cannot run the CLI and see:
--   ERROR: column "status" of relation "hackathons" does not exist
-- This script only adds missing pieces; it does not drop data.
...
-- Ensure base table exists (unlikely to be missing if seed reached INSERT)
...
-- Backfill required fields (existing rows only)
...
-- Submissions + judge_scores (for app features); safe if already present
